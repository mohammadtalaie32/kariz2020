<?php $__env->startSection("content"); ?>
    ...
    <fieldset style="border: 2px solid lightgray;padding:30px;max-width:100%;">
        <form method="POST" action="/admin/add_users/update/<?php echo e($user->id); ?>" id="users">
            ...
            <br>
            <input type="text" size="50" name="name" style="font-size:150%;border:2px solid gray;border-radius:10px;max-width:100%" value="<?php echo e($user->name); ?>" required>
            <br><br>
            <label for="email" style="font-size:200%"> ایمیل کاربر : </label>
            <br>
            <input type="email" size="50" name="email" style="font-size:150%;border:2px solid gray;border-radius:10px;max-width:100%" value="<?php echo e($user->email); ?>" required>


            <br><br>

            <label for="roles">نقش کاربر : </label><br>
            <?php if(($user->Role)[0]["role_name"] == "admin"): ?>
            ...
            <?php elseif(($user->Role)[0]["role_name"] == "teacher"): ?>
                <select id="roles" name="roles" form="users" class="browser-default custom-select">
            ...
            <?php elseif(($user->Role)[0]["role_name"] == "student"): ?>
                <select id="roles" name="roles" form="users" class="browser-default custom-select">
                    <option value="admin" id="id1">Admin</option>
                    <option value="teacher" id="id2" >Teacher</option>
